...
import { X, User, Mail, Camera, KeyRound, Loader2, Check, Sparkles } from 'lucide-react';
...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200/80 dark:border-slate-800 w-full max-w-md p-6 relative overflow-hidden transition-colors duration-200">
        {/* Header Bar */}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="flex flex-col items-center justify-center p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800">
            <img
              src={avatarUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${name}&backgroundColor=7c3aed&textColor=ffffff`}
              alt={name}
              className="w-16 h-16 rounded-full bg-brand-100 border-2 border-brand-500 object-cover shadow-md shadow-brand-500/20 mb-3"
            />

            <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-brand-500" /> Choose Preset Avatar
            </p>
